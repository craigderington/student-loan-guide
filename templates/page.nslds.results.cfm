			...
			<cfif structkeyexists( url, "fuseaction" ) and url.fuseaction is "reset">				
				<cfif structkeyexists( url, "nslid" ) and isvalid( "integer", url.nslid ) >
					...
					<cfquery datasource="#application.dsn#" name="resetconverted">
						update nslds
						 ...
						 where nslid = <cfqueryparam value="#nslid#" cfsqltype="cf_sql_int" />
					</cfquery>
				</cfif>
			...
			<cfif structkeyexists( form, "createworksheets" ) and structkeyexists( form, "__authToken" ) >
				...
				<cfif not structkeyexists( form, "chkDebt" )>				
			
					<script>
						self.location="index.cfm?event=page.nslds.results&alert=nochkdebt"
					</script>
				...
				<div class="container">
					...
					<div class="row">
						...
						<div class="span12">
							...
							<cfif structkeyexists( session, "nsldsresult")>
								<cfoutput>
									<div class="alert alert-box alert-info">
										<a class="close" data-dismiss="alert">&times;</a>
											<h5><i class="icon-paste"></i> NSLDS IMPORT RESULTS!</h5>
											<ul>
												<li>#session.nsldsresult#</li>
												<li>By default, zero balance loans are not shown below.  Use the toggle to show zero balance loans</li>
											</ul>
									</div>
								</cfoutput>
							</cfif>
							<cfif structkeyexists( url, "alert" ) and trim( url.alert ) eq "nochkdebt" >								
									<div class="alert alert-box alert-error">
										<a class="close" data-dismiss="alert">&times;</a>
											<h5><i class="icon-warning-sign"></i> NO DEBTS SELECTED!</h5>
											<ul>												
												<li>It appears you clicked the button to Create Debt Worksheets, but did not select any debts to include.  Please try again...</li>
											</ul>
									</div>								
							</cfif>
							...
							<div class="widget stacked">
								...
								<div class="widget-content">							
									...
									<div class="well">
										...
										<form class="form-inline" name="filterresults" method="post">
											<select name="loantypes" style="margin-left:5px;" class="input-xlarge" onchange="javascript:this.form.submit();">
												...
												<cfloop query="loantypes">
													<option value="#nslloantype#"<cfif isdefined( "form.loantypes" ) and form.loantypes eq loantypes.nslloantype>selected</cfif>>#nslloantype#</option>
												</cfloop>												
											</select>
										...
										<cfset counter = 1 />
										<cfoutput>
											<h5><i class="icon-folder-open"></i> Showing #nsldslist.recordcount# NSLDS Loan<cfif nsldslist.recordcount gt 1>s</cfif>  <span class="pull-right"><cfif not structkeyexists( url, "dozerobal" )><cfif nsldszerobal.recordcount GT 0><i class="icon-warning-sign" style="color:red;margin-left:10px;"></i> Not showing #nsldszerobal.recordcount# loan<cfif nsldszerobal.recordcount gt 1>s</cfif> with zero balance<cfif nsldszerobal.recordcount gt 1>s</cfif>.</cfif></cfif>  <cfif not structkeyexists( url, "dozerobal" )><a style="margin-left:10px;" href="#application.root#?event=#url.event#&dozerobal=1">Show Zero Balance Loans</a><cfelse><a style="margin-left:10px;" href="#application.root#?event=#url.event#">Hide Zero Balance Loans</a></cfif></span></h5>
										</cfoutput>
											...
											<fieldset>
												<table class="table table-bordered table-striped table-highlight">
													...
													<tbody>
														...
														<tr>
															<td class="actions"><div align="center"><cfif converted eq 0><input type="checkbox" name="chkdebt" value="#nslid#"><cfelse><cfif isuserinrole( "admin" ) or isuserinrole( "co-admin" )><a href="#application.root#?event=#url.event#&fuseaction=reset&nslid=#nslid#" title="Reset Converted" class="btn btn-tertiary btn-mini" onclick="return confirm('This will reset the NSLDS record so it can be imported again, do you wish to continue?');"><i class="btn-icon-only icon-unlock"></i></a><cfelse><a href="javascript;:" rel="popover" data-original-title="NSLDS Loan Converted" data-content="This student loan can not be selected because it has already been converted to a debt worksheet."><i class="icon-info-sign" style="color:##ff0000;"></i></a></cfif></cfif></div></td>														
															<td>#counter#</td>
															<td>#nslloantype#</td>
															<td>#nslloanstatus#</td>
