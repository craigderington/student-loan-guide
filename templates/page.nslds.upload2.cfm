			...
			<!--- // 1-2-2015 // Change to upload to client folder, then call .NET component --->
			...
			<cfelseif structkeyexists( session, "nsldsresult" )>
				<cfparam name="tempZ" default="">
				<cfset tempZ = structdelete( session, "nsldsresult" ) />			
			</cfif>
			...
			<!--- // dot net path --->
			<cfparam name="dotnetpath" default="#expandpath('apis/dotnet/nslds/NSLDSClassLibrary.dll')#">
						...
						<div class="span12">
							...
							<div class="widget stacked">
								...
								<div class="widget-header">		
									...
									<h3>Process NSLDS My Student Loan Data Text File for #leaddetail.leadfirst# #leaddetail.leadlast#</h3>						
								</div> <!-- //.widget-header -->
								</cfoutput>	
								...
								<div class="widget-content">								
									...
									<cfif isDefined( "form.fieldnames" )>
										...
										<cfif objValidation.getErrorCount() is 0>							
											...
											<cfelse>
												...
												<cffile action="upload" nameconflict="overwrite" filefield="fileuploadpath" destination="#expandpath( 'library\clients\enrollment\' & session.leadid & '-My-Student-Data.txt' )#">
												
												<!--- // pause the system to allow the server write operation to complete
												     // before calling the .NET component to read in the file
													 // to the servers memory (sleep: in milliseconds; 1000ms = 1 second )
													 --->												
												<cfscript>
												...
												<!--- // call new .NET component --->
												
												<cfobject
													type=".NET"		
													class="NSLDSFixed.Upload"
													assembly="#dotnetpath#"
													name="nslds">
												
													<cfset nslds.init() />												
													<cfset leadid = session.leadid />
													<cfset thisnslds = nslds.UploadNSLDS( javacast( "int", leadid )) />
													<!---<cfset thisnslds = nslds.UploadNSLDS( leadid ) />--->
													<cfset thisresponse = thisnslds />
													<cfset session.nsldsresult = thisresponse />													
												...
												<cfset newLine	 = chr(13) & chr(10) />			
													...
													<cflocation url="#application.root#?event=page.nslds.results" addtoken="yes">
									...
									<h4><i class="icon-upload-alt"></i> Process NSLDS | Step 1</h4> 
									
									<p>This form will allow you to upload and analyze the National Student Loan Data System (NSLDS) text file generated from the NSLDS website.  Only official Department of Education NSLDS text files may be used to create student loan debt worksheets.</p>
