				...
				<div class="container">
					...
					<div class="row">
						...
						<div class="span12">
							...
							<cfif structkeyexists( url, "s2" ) and trim( url.s2 ) is "sent">
								<cfoutput>
									<div class="alert alert-box alert-success">
										<button type="button" class="close" data-dismiss="alert">&times;</button>
										<strong><i class="icon-envelope"></i> EMAIL SENT!</strong>  The client email notification was sent successfully at #dateformat( now(), "mmm d, yyyy" )# at #timeformat( now(), "hh:mm:ss tt" )#.
									</div>
								</cfoutput>
							</cfif>
									...
									<cfif isDefined("form.fieldnames")>
										...
										<cfif objValidation.getErrorCount() is 0>
											...
											<cfif structkeyexists( form, "createleadlogin" )>
												...
												<cfquery datasource="#application.dsn#" name="checkusername">
													...
													<cfelse>
															...
															<cfif isvalid( "email", lead.email ) and isvalid( "email", companysettings.email )>							
																<cfmail from="#companysettings.email# (#companysettings.companyname#)" to="#lead.email#" subject="Student Loan Counseling Portal Available" type="HTML">																			
																...
																<cfset emsg = "Client Portal Login Email sent successfully..." />
															</cfif>
											...
											<cfelseif structkeyexists( form, "saveleadlogin" )>
												...
												<!--- // check to see if the user wants to re-send the email to the client --->
												<cfif structkeyexists( form, "resendinvite" )>													
													<cfif form.resendinvite eq 1>
													<!--- // 12-29-2014 // modify to allow for dynamic email templates --->					
														<cfif isvalid( "email", lead.email ) and isvalid( "email", companysettings.email )>							
															<cfmail from="#companysettings.email# (#companysettings.companyname#)" to="#lead.email#" subject="Student Loan Counseling Portal Available" type="HTML">																			
																<!--- // include the dynamic body // client-login-email-template.cfm in company library path --->
											...
											<cfif structkeyexists( form, "createleadlogin" )>												
												<cflocation url="#application.root#?event=#url.event#&msg=saved&s2=sent" addtoken="no">												
											<cfelseif structkeyexists( form, "saveleadlogin" )>
												<cfif structkeyexists( form, "resendinvite" )>												
													<cflocation url="#application.root#?event=#url.event#&msg=saved&s2=sent" addtoken="no">
												<cfelse>
															...
															<form id="edit-lead-login-settings" class="form-horizontal" method="post" action="#application.root#?event=#url.event#">
																...
																<fieldset>																
																	...
																	<cfif qleadlogin.recordcount neq 0>
																		...
																		<div class="control-group">											
																			<label class="control-label" for="resendinvite">Re-Send Email Invite</label>
																			<div class="controls">
																				<input type="checkbox" class="input-medium" name="resendinvite" id="resendinvite" value="1" />
																			</div> <!-- /controls -->				
																		</div> <!-- /control-group -->
																	<cfelse>
