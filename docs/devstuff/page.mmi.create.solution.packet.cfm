		...
		<cfquery datasource="#application.dsn#" name="updateworksheets">
			update slworksheet
			 ...
			 <!--- // modify for MMI 
			 worksheetid IN( <cfqueryparam value="#thisworksheet#" cfsqltype="cf_sql_integer" list="yes" /> )
			 --->
		</cfquery>
		...
		<cfinvoke component="apis.com.tasks.taskautomation" method="marktaskcompleted" returnvariable="taskmsg">
			...
			<cfdocument
				...
				<cfoutput>	
					...
					<!--- 
							// 1-24-2015 *** CLD
							// modify solution presenation for MMI workflow
							// and presenation options.  they have opted to
							// give the client all of the options of which 
							// qualified in the option trees
					--->
					<cfdocumentsection name="Action Plan">
						...
						<cfdocumentitem type="footer">
							...
							<div style="margin-top:10px;">							
								...
								<cfloop query="mmisolutions">						
									
									
									<!--- // do stuff here to manipulate the strings
									      // for use in the show plan below --->
									...
									--->
										select ap.actionplanid, ap.optiontree, ap.optiondescr, ap.optionsubcat, ap.actionplanheader, ap.actionplanbodya
										  from actionplan ap
										 where optiontree <> <cfqueryparam value="Advisory" cfsqltype="cf_sql_varchar" />
										   and ap.optiondescr IN(<cfqueryparam value="#mmisolutions#" cfsqltype="cf_sql_varchar" list="yes" />)
										   and ap.optionsubcat IN(<cfqueryparam value="#mmisolutions#" cfsqltype="cf_sql_varchar" />)
										   and ap.optiontree LIKE <cfqueryparam value="%#tree#%" cfsqltype="cf_sql_varchar" />
									  order by ap.tocorder asc
									</cfquery>
